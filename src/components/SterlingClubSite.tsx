...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-cyan-500/30 dark:border-cyan-300/20 bg-cyan-50 dark:bg-cyan-300/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.28em] text-cyan-600 dark:text-cyan-200">
    ...
    <div className={dark ? "dark" : ""}>
      <main className="min-h-screen overflow-hidden bg-slate-50 dark:bg-[#050608] text-slate-900 dark:text-white">
        ...
        <div className="pointer-events-none fixed inset-0 opacity-25 dark:opacity-70">
          <div className="absolute left-[-12%] top-[-12%] h-[42rem] w-[42rem] rounded-full bg-cyan-500/10 blur-3xl" />
          <div className="absolute right-[-10%] top-[20%] h-[36rem] w-[36rem] rounded-full bg-amber-400/10 blur-3xl" />
          <div className="absolute bottom-[-15%] left-[35%] h-[30rem] w-[30rem] rounded-full bg-blue-700/20 blur-3xl" />
        </div>

        {/* Nav */}
        <nav className="relative z-20 mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
          <div className="flex items-center gap-4">
            <div className="rounded-2xl border border-amber-500/20 dark:border-amber-200/15 bg-white dark:bg-black/40 px-4 py-3 shadow-lg shadow-slate-200/40 dark:shadow-xl dark:shadow-black/30">
              <img
                src="/sterling-club-logo.svg"
                alt="The Sterling Club"
                className="h-8 w-auto object-contain"
              />
            </div>
            <div className="hidden sm:block">
              <div className="text-sm font-bold tracking-[0.28em] text-slate-800 dark:text-amber-100">TPG / VIBE100</div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="rounded-full bg-slate-900 dark:bg-white px-5 py-3 text-sm font-bold text-white dark:text-black transition hover:bg-slate-700 dark:hover:bg-cyan-100">
              Request Access
            </button>
          </div>
        </nav>
        ...
        <section className="relative z-10 mx-auto grid min-h-[86vh] max-w-7xl items-center gap-12 px-6 py-14 lg:grid-cols-[1.08fr_0.92fr]">
          <motion.div {...fadeUp}>
            <div className="mb-8 inline-flex rounded-3xl border border-amber-500/20 dark:border-amber-200/15 bg-white dark:bg-black/35 px-6 py-4 shadow-lg shadow-slate-200/40 dark:shadow-2xl dark:shadow-black/40 backdrop-blur">
              <img
                src="/sterling-club-logo.svg"
                alt="The Sterling Club"
            ...
            <h1 className="max-w-4xl text-5xl font-black leading-[0.95] tracking-tight text-slate-900 dark:text-white md:text-7xl lg:text-8xl">
              Control the room.{" "}
              <span className="text-cyan-600 dark:text-cyan-200">Monetize the ecosystem.</span>
            </h1>
            ...
            <div className="mt-9 flex flex-col gap-4 sm:flex-row">
              <button className="group rounded-2xl bg-cyan-600 dark:bg-cyan-300 px-7 py-4 font-black text-white dark:text-black shadow-2xl shadow-cyan-950/20 dark:shadow-cyan-950/50 transition hover:bg-cyan-700 dark:hover:bg-cyan-200">
          ...
          <motion.div {...fadeUp} className="relative">
            <div className="absolute -inset-8 rounded-[2.5rem] bg-gradient-to-br from-cyan-300/10 via-white/5 to-amber-300/10 dark:from-cyan-300/15 dark:via-white/5 dark:to-amber-300/15 blur-2xl" />
            <Card className="relative p-4">
              <div className="aspect-[4/5] overflow-hidden rounded-[1.7rem] border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-[radial-gradient(circle_at_30%_20%,rgba(103,232,249,.22),transparent_28%),linear-gradient(145deg,#111827,#030712_58%,#14120b)] p-5">
                ...
                <div className="mt-14 rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-black/35 p-5 backdrop-blur">
                  <div className="flex items-center gap-3">
                    <div className="grid h-12 w-12 place-items-center rounded-2xl bg-cyan-600 dark:bg-cyan-300 text-white dark:text-black">
                      <Network />
                    </div>
                    <div>
                    ...
                    {rooms.map((r) => {
                      ...
                      return (
                        <div key={r.name} className="flex gap-3 rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 p-4">
                          <Icon className="mt-1 text-cyan-600 dark:text-cyan-200" size={20} />
                          <div>
            ...
            {[
              ...
              <motion.div key={title as string} {...fadeUp}>
                <Card className="h-full">
                  {React.createElement(Icon as React.ElementType, { className: "text-cyan-600 dark:text-cyan-200", size: 30 })}
                ...
                {sponsorTiers.map((s, i) => (
                  <button
                    ...
                    className={`flex items-center justify-between rounded-2xl border p-4 text-left transition ${
                      selectedTier === i
                        ? "border-cyan-500/50 dark:border-cyan-300/50 bg-cyan-50 dark:bg-cyan-300/10 text-slate-900 dark:text-white"
            ...
            <motion.div {...fadeUp}>
              <Card className="h-full p-8">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <div className="text-sm uppercase tracking-[0.3em] text-cyan-600 dark:text-cyan-200">Selected tier</div>
                    <h3 className="mt-2 text-4xl font-black text-slate-900 dark:text-white">{tier.name}</h3>
                  </div>
                  <div className="rounded-2xl bg-slate-900 dark:bg-white px-4 py-3 text-xl font-black text-white dark:text-black">{tier.range}</div>
                </div>
                ...
                <div className="mt-8 grid gap-4 md:grid-cols-3">
                  {tier.perks.map((p) => (
                    <div key={p} className="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/25 p-4">
                      <CheckCircle2 className="text-cyan-600 dark:text-cyan-200" size={20} />
                      <div className="mt-3 text-sm font-bold text-slate-700 dark:text-white/75">{p}</div>
                    </div>
                  ))}
                </div>
                <div className="mt-8 rounded-3xl border border-amber-500/20 dark:border-amber-200/20 bg-amber-50 dark:bg-amber-200/10 p-5 text-sm leading-6 text-amber-900 dark:text-amber-50/80">
          ...
          <motion.div {...fadeUp} className="grid gap-10 lg:grid-cols-[1fr_1fr] lg:items-center">
            <div>
              ...
              <div className="mt-8 rounded-3xl border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-white/[0.04] p-5">
                <div className="flex items-center gap-3 text-cyan-600 dark:text-cyan-200">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {metrics.map((m) => (
                <Card key={m.metric}>
                  <div className="text-3xl font-black text-cyan-600 dark:text-cyan-200">{m.metric} ↑</div>
              ...
              {calendar.map((c) => {
                ...
                return (
                  <motion.div key={c.label} {...fadeUp}>
                    <Card className="h-full">
                      <div className="flex items-center justify-between">
                        <div className="rounded-xl bg-cyan-600 dark:bg-cyan-300 px-3 py-1 text-sm font-black text-white dark:text-black">{c.label}</div>
            ...
            <Card className="grid gap-5 md:grid-cols-3">
              {[
                ...
                <div key={title as string} className="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/25 p-5">
                  {React.createElement(Icon as React.ElementType, { className: "text-cyan-600 dark:text-cyan-200" })}
            ...
            <div className="mt-12 grid gap-5 md:grid-cols-3">
              {[
                ...
                <Card key={title as string}>
                  {React.createElement(Icon as React.ElementType, { className: "text-amber-600 dark:text-amber-200", size: 30 })}
                  <h3 className="mt-5 text-xl font-black text-slate-900 dark:text-white">{title as string}</h3>
                  <p className="mt-3 text-sm leading-6 text-slate-500 dark:text-white/55">{copy as string}</p>
                </Card>
              ))}
        ...
        <section className="relative z-10 mx-auto max-w-5xl px-6 py-24 text-center">
          <motion.div {...fadeUp}>
            <div className="mx-auto mb-7 grid h-16 w-16 place-items-center rounded-3xl border border-cyan-500/25 dark:border-cyan-300/25 bg-cyan-50 dark:bg-cyan-300/10">
              <Crown className="text-cyan-600 dark:text-cyan-200" />
            </div>
            ...
            <div className="mt-10 flex flex-col justify-center gap-4 sm:flex-row">
              <button className="rounded-2xl bg-slate-900 dark:bg-white px-8 py-4 font-black text-white dark:text-black transition hover:bg-slate-700 dark:hover:bg-cyan-100">
        ...
        <footer className="relative z-10 border-t border-slate-200 dark:border-white/10 px-6 py-10 text-center text-sm text-slate-400 dark:text-white/45">
          <div className="mx-auto mb-5 inline-flex rounded-2xl border border-amber-500/20 dark:border-amber-200/15 bg-white dark:bg-black/40 px-5 py-3">
            <img src="/sterling-club-logo.svg" alt="The Sterling Club" className="h-7 w-auto object-contain" />
          </div>
